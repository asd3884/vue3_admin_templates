...
注意：配置husky之前，1.在项目根目录下执行 git init 初始化git仓库2.在github上创建一个远程仓库 名为：vue3_admin_templates 3.将本地仓库与远程仓库关联：
...
自此，项目中svg的使用：1.在src/assets目录下创建一个icons文件夹2.在阿里图标库中复制svg代码 如phone.svg，将phone.svg放入到 src/assets/icons目录下
3.svg的使用如下：
在App.vue中：
...
在入口文件引入index.ts文件,通过app.use方法安装自定义插件
...
在App组件中使用：
<svgIcon name="love" color="pink"></svgIcon>
...
注意：我当前开发环境的配置文件中的VITE_APP_BASE_API:/api
在App组件中做一个测试
